...
def plot_blackscholes( world, gym, config, strike : float = 1., iscall : bool = True ):
    ...
    utilityBS = gym.utility.compute_stateless_utility( X=bspnl + payoff, sample_weights=world.sample_weights )
    utilityDH2 = gym.utility.compute_stateless_utility( X=dhpnl + payoff, sample_weights=world.sample_weights )
    ...
    plt_termpayoff.plot( bin_spotT, -bin_eff   -dprice,  "*-", color="orange", label="-model hedge pnl")
    plt_termpayoff.plot( bin_spotT, -bin_bseff -dprice,  "-",  color="green", label="-bs hedge pnl")
#deep hedging    plt_termpayoff.plot( bin_spotT, -bin_dheff -dprice,  ":",  color="black", label="-hedged pnl from DH")
    ...
    plt_utility.bar( ['Unhedged','BS','DH','DH2'], [ utility0, utilityBS, utility,utilityDH2 ] )
